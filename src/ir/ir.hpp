...
class Type
{
  public:
    ...
    {
        return tid_ == IntegerTID || tid_ == RealTID || tid_ == CharTID || tid_ == BooleanTID;
    }
    virtual TID get_tid() const
    {
        return tid_;
    }
    TID tid_;
...
{
  public:
    explicit ArrayType(std::shared_ptr<Type> elem_type, std::vector<unsigned> dims_elem_num)
        : Type(Type::ArrayTID), elem_type_(elem_type), dims_elem_num_(dims_elem_num)
    {
    }
    virtual std::string print() override
    {
...
class FunctionType : public Type
{
  public:
    ...
    virtual std::string print() override
    {
        return result_->print();
    }
    virtual std::string placeholder() override
    {
        return result_->placeholder();
    }
    bool is_number() const override
    {
        return result_->is_number();
    }
    TID get_tid() const override
    {
        return result_->tid_;
    }
...
class Value
{
  public:
    ...
    explicit Value(std::shared_ptr<Type> type, const Value::ValueID id = ValueID::Value, const std::string name = "")
        : type_(type), val_id_(id), name_(name)
    {
    }
    ~Value() = default;
    virtual std::string print()
    ...
    */
    ...
    {
        use_list_.remove_if([val](Use u) { return u.val_.lock() == val; });
    }
...
class GlobalIdentifier : public Value
{
  public:
     ...
     * @param init_val
     */
    GlobalIdentifier(std::shared_ptr<Type> type, const std::string name, bool is_const,
...
{
  public:
    LocalIdentifier(std::shared_ptr<Type> type, const std::string name, bool is_const,
                    std::shared_ptr<Literal> init_val);
...
{
  public:
    explicit Argument(std::shared_ptr<Type> type, const std::string name, unsigned arg_no = 0)
        : Value(type, ValueID::Argument, name), arg_no_(arg_no)
    {
    }
    ~Argument()
    {
...
class Function : public Value, public std::enable_shared_from_this<Function>
{
  public:
     ...
     * @param l 局部标识符
     */
    void add_local_identifier(std::shared_ptr<LocalIdentifier> l)
    {
        local_identifiers_.emplace_back(l);
    }
...
class BasicBlock : public Value
{
  public:
    explicit BasicBlock(const std::string name)
        : Value(std::make_shared<Type>(Type::BlockTID), ValueID::BasicBlock, name)
    {
    }
    ~BasicBlock() = default;
    virtual std::string print() override
     ...
     *
     */
    void add_instruction(std::shared_ptr<Instruction> i)
    {
        instructions_.emplace_back(i);
    }
     ...
     *
     */
    void add_pre_bb(std::weak_ptr<BasicBlock> bb)
    {
        pre_bbs_.emplace_back(bb);
    }
    /**
     * @brief 添加后继基本块
     */
    void add_succ_bb(std::weak_ptr<BasicBlock> bb)
    {
        succ_bbs_.emplace_back(bb);
    }
...
class Instruction : public Value
{
  public:
    ...
    explicit Instruction(std::shared_ptr<Type> ty, OpID id, unsigned num_ops, std::weak_ptr<BasicBlock> bb,
                         bool before = false);
    ...
    bool is_binary_inst()
    {
        return op_id_ >= OpID::Add && op_id_ <= OpID::In;
    }
    bool is_unary_inst()
    {
        return op_id_ >= OpID::Not && op_id_ <= OpID::LogicalNot;
    }
    bool is_assign_inst()
    {
        return op_id_ == OpID::Assign;
    }
    bool is_load_inst()
    {
        return op_id_ == OpID::Visit;
    }
    bool is_expr()
    {
        return is_binary_inst() || is_unary_inst() || is_assign_inst() || is_load_inst();
    }
    ...
    std::vector<std::list<Use>::iterator>
        use_pos_; // 与操作数数组一一对应，是对应的操作数的 uselist 里面，与当前指令相关的 use 的迭代器
    std::vector<std::shared_ptr<ir::Instruction>>::iterator
        pos_in_bb_;                // 在 bb 的指令 list 的位置迭代器, 最多只能有一个
...
class UnaryInst : public Instruction
{
  public:
     ...
     */
    UnaryInst(std::shared_ptr<Type> ty, OpID op, std::shared_ptr<Value> val, std::weak_ptr<BasicBlock> bb)
...
class CompareInst : public Instruction
{
  public:
     ...
     */
    CompareInst(OpID op, std::shared_ptr<Value> v1, std::shared_ptr<Value> v2, std::weak_ptr<BasicBlock> bb)
...
class StoreInst : public Instruction
{
  public:
     ...
     */
    StoreInst(std::shared_ptr<Value> val, std::shared_ptr<Value> ptr, std::weak_ptr<BasicBlock> bb)
...
{
  public:
    ReadInst(std::vector<std::shared_ptr<Value>> args, std::weak_ptr<BasicBlock> bb)
...
{
  public:
    WriteInst(std::vector<std::shared_ptr<Value>> args, std::weak_ptr<BasicBlock> bb)
...
class CallInst : public Instruction
{
  public:
     ...
     * @details 用于调用函数
     */
    CallInst(std::shared_ptr<Function> func, std::vector<std::shared_ptr<Value>> args, std::weak_ptr<BasicBlock> bb)
...
class ReturnInst : public Instruction
{
  public:
     ...
     * @details 用于返回
     */
    ReturnInst(std::shared_ptr<Value> val, std::weak_ptr<BasicBlock> bb)
...
class ContinueIncInst : public Instruction
{
  public:
     ...
     * @param bb 所属基本块
     */
    ContinueIncInst(std::shared_ptr<BasicBlock> cont_bb, std::shared_ptr<BasicBlock> jmp_bb,
                    std::weak_ptr<BasicBlock> bb)
...
class Module
{
  public:
     ...
     * @param l 常量
     */
    void add_literal(std::shared_ptr<Literal> l)
    {
        all_literals_.emplace_back(l);
    }
    ...
    void add_instruction(std::shared_ptr<Instruction> i)
    {
        all_instructions_.emplace_back(i);
    }
